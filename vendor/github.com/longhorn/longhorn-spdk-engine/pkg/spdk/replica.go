...
func (r *Replica) validateAndUpdate(bdevLvolMap map[string]*spdktypes.BdevInfo, subsystemMap map[string]*spdktypes.NvmfSubsystem) (err error) {
	defer func() {
		if err != nil && r.State != types.InstanceStateError {
			r.State = types.InstanceStateError
			r.log.WithError(err).Errorf("Found error during validation and update")
		}
	}()
...
func (r *Replica) Delete(spdkClient *SPDKClient, cleanupRequired bool, superiorPortAllocator *util.Bitmap) (err error) {
	...
	defer func() {
		if err != nil {
			r.State = types.InstanceStateError
			r.log.WithError(err).Errorf("Failed to delete replica %s", r.Name)
		}
...
func (r *Replica) SnapshotShallowCopy(spdkClient *SPDKClient, snapshotName string) (err error) {
	r.RLock()
...
func (r *Replica) RebuildingDstStart(spdkClient *SPDKClient, exposeRequired bool) (address string, err error) {
	...
	defer func() {
		if err != nil && r.State != types.InstanceStateError {
			r.State = types.InstanceStateError
			r.log.WithError(err).Errorf("Failed to start rebuilding for destination replica %s", r.Name)
			updateRequired = true
		}
	}()
...
func (r *Replica) RebuildingDstFinish(spdkClient *SPDKClient, unexposeRequired bool) (err error) {
	...
	// Then re-construct the replica ActiveChain (and SnapshotMap)
	// TODO: Can we somehow rely on the periodic sync to re-construct the chain?
...
func (r *Replica) RebuildingDstSnapshotCreate(spdkClient *SPDKClient, snapshotName string) (err error) {
	...
	defer func() {
		if err != nil && r.State != types.InstanceStateError {
			r.State = types.InstanceStateError
			r.log.WithError(err).Errorf("Failed to create rebuilding snapshot %s for destination replica %s", snapshotName, r.Name)
			updateRequired = true
		}
	}()
