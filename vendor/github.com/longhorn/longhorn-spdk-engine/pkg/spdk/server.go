...
func (s *Server) EngineCreate(ctx context.Context, req *spdkrpc.EngineCreateRequest) (ret *spdkrpc.Engine, err error) {
	...
	return e.Create(s.spdkClient, req.ReplicaAddressMap, s.getLocalReplicaLvsNameMap(req.ReplicaAddressMap), req.PortCount, req.UpgradeRequired, s.portAllocator)
}
...
func (s *Server) EngineSuspend(ctx context.Context, req *spdkrpc.EngineSuspendRequest) (ret *empty.Empty, err error) {
	...
	if err := e.Suspend(s.spdkClient, s.portAllocator); err != nil {
		return nil, err
	}
