...
func (e *Engine) ValidateAndUpdate(
	...
	if err := e.validateAndUpdateFrontend(subsystemMap); err != nil {
		if strings.Contains(err.Error(), "connecting state") ||
			...
			e.log.WithError(err).Warnf("Failed to validate and update frontend for engine %s", e.Name)
			return nil
		}
		return err
	}
...
func (e *Engine) validateAndUpdateFrontend(subsystemMap map[string]*spdktypes.NvmfSubsystem) (err error) {
	...
	if e.Frontend == types.FrontendEmpty {
		if subsystem != nil {
			return fmt.Errorf("found nvmf subsystem %s for engine %s with void frontend", nqn)
		}
		if e.Endpoint != "" {
			return fmt.Errorf("found non-empty endpoint %s for engine %s with void frontend", e.Endpoint)
		}
		if e.Port != 0 {
			return fmt.Errorf("found non-zero port %s for engine %s with void frontend", e.Port)
		}
		return nil
	}
...
func (e *Engine) ReplicaShallowCopy(dstReplicaName, dstReplicaAddress string) (err error) {
	...
	for currentSnapshotName := stack[len(stack)-1]; len(stack) > 0; {
		...
		hasChildSnap := false
		for childSnapName := range rpcSrcReplica.Snapshots[currentSnapshotName].Children {
			if childSnapName == srcReplicaName {
				continue
			}
			stack = append(stack, childSnapName)
			hasChildSnap = true
		}
		if !hasChildSnap {
