...
func (s *Server) DiskDelete(ctx context.Context, req *rpc.DiskDeleteRequest) error {
	log := logrus.WithFields(logrus.Fields{
		"diskType": req.DiskType,
	})
	...
	if req.DiskName == "" {
		return grpcstatus.Error(grpccodes.InvalidArgument, "disk name is required")
	}
	...
	switch req.DiskType {
	case rpc.DiskType_block:
		return s.blockTypeDiskDelete(ctx, req)
	default:
		return grpcstatus.Errorf(grpccodes.Unimplemented, "unsupported disk type %v", req.DiskType)
	}
}
