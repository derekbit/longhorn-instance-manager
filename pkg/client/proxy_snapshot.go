...
func (c *ProxyClient) SnapshotHashInProgressTasks(serviceAddress string) (numTasks int, err error) {
	input := map[string]string{
		"serviceAddress": serviceAddress,
	}
	if err := validateProxyMethodParameters(input); err != nil {
		return -1, errors.Wrap(err, "failed to get snapshot hash status")
	}
	...
	recv, err := c.service.SnapshotHashInProgressTasks(getContextWithGRPCTimeout(c.ctx), req)
	if err != nil {
		return -1, err
	}

	return int(recv.NumTasks), nil
}
