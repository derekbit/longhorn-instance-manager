...
func (c *GRPCHealthChecker) WaitForRunning(address, name string, stopCh chan struct{}) bool {
	...
	for i := 0; i < types.WaitCount; i++ {
		select {
		case <-stopCh:
			logrus.Infof("Stop waiting for gRPC service of spdk service %v to start at %v", name, address)
			return false

		case <-ticker.C:
			if c.IsRunning(address) {
				logrus.Infof("Spdk service %v has started at %v", name, address)
				return true
			}
			logrus.Infof("Wait for gRPC service of spdk service %v to start at %v", name, address)
		}
	}
