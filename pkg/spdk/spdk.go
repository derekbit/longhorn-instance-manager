...
func (s *Server) ReplicaGet(ctx context.Context, req *rpc.ReplicaGetRequest) (*rpc.Replica, error) {
	...
	localPort := 0
	controllerInfos, err := spdkClient.BdevNvmeGetControllers(req.Name)
	if err != nil {
		resp, parseErr := parseErrorMessage(err.Error())
		if parseErr != nil || !strings.Contains(resp.Message, "does not exist") {
			log.WithError(err).Error("Failed to get controller info")
			return nil, grpcstatus.Error(grpccodes.Internal, errors.Wrap(err, "failed to get controller info").Error())
		}
	} else {
		controller := controllerInfos[0]
		logrus.Infof("Debug ==> controller.Ctrlrs[0].Trid.Trsvcid=%v", controller.Ctrlrs[0].Trid.Trsvcid)

		localPort, err = strconv.Atoi(controller.Ctrlrs[0].Trid.Trsvcid)
		if err != nil {
