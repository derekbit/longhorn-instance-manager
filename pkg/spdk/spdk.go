...
func (s *Server) ReplicaDelete(ctx context.Context, req *rpc.ReplicaDeleteRequest) (*empty.Empty, error) {
	...
	lvols, err := spdkClient.BdevLvolGet("", 0)
	if err != nil {
		log.WithError(err).Error("Failed to get lvol")
		return nil, grpcstatus.Error(grpccodes.Internal, errors.Wrap(err, "failed to get lvol").Error())
	}

	for _, lvol := range lvols {
		aliasName := lvol.Aliases[0]
		if strings.Contains(aliasName, req.Name) {
			log.Infof("Deleting lvol %v", aliasName)
			deleted, err := spdkClient.BdevLvolDelete(lvol.Name)
			if err != nil {
			...
			if !deleted {
				log.Errorf("Failed to delete lvol %v", lvol.Name)
				return nil, grpcstatus.Error(grpccodes.Internal, errors.Wrapf(err, "failed to delete lvol %v", lvol.Name).Error())
			}
			break
		}
	}
