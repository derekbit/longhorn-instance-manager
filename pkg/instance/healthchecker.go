...
func (c *GRPCHealthChecker) WaitForRunning(address, name string, stopCh chan struct{}) bool {
	...
	for i := 0; i < types.WaitCount; i++ {
		select {
		case <-stopCh:
			logrus.Infof("Stop waiting for gRPC service of instance service %v to start at %v", name, address)
			return false
		...
		case <-ticker.C:
			...
			logrus.Infof("Wait for gRPC service of instance service %v to start at %v", name, address)
		}
	}
