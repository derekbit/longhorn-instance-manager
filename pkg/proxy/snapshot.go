...
func (p *Proxy) SnapshotHashInProgressTasks(ctx context.Context, req *rpc.ProxyEngineRequest) (resp *rpc.EngineSnapshotHashInProgressTasksProxyResponse, err error) {
	log := logrus.WithFields(logrus.Fields{"serviceURL": req.Address})
	log.Debug("Getting in-progress snapshot hash tasks")
	...
	numTasks, err := task.HashSnapshotInProgressTasks()
	if err != nil {
		return nil, err
	}

	return &rpc.EngineSnapshotHashInProgressTasksProxyResponse{
